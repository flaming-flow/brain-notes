...
### Генерация контента для Threads

Бот генерирует посты для Threads из твоих заметок:

```
/generate threads [тема]     — генерация по конкретной теме
/generate threads             — AI предложит 5 тем из твоих заметок
```

**Форматы постов** (AI выбирает автоматически или можно переключить кнопкой):
- **Insight** — личное открытие или наблюдение
- **Hot Take** — спорное мнение, приглашающее к дискуссии
- **Question** — вопрос из личного опыта
- **Story** — конкретный момент → что произошло → что понял
- **Framework** — простой принцип или ментальная модель

**Voice Samples** — система обучения стилю:
- Нажми "Save as example" на понравившемся посте
- Бот сохраняет его как образец стиля
- До 5 последних примеров инжектятся в промпт
- Чем больше примеров, тем точнее AI копирует твой голос

**Правила генерации:**
- Русский язык, до 500 символов
- Хук в первой строке (< 15 слов)
- Без корпоративного тона и AI-клише
- Topic tags вместо хэштегов (один на пост)

```
Пользователь: /generate threads танец как медитация

Бот: Вчера на танцполе поймал момент, когда мысли просто выключились.

Не пустота — а наоборот, полнота. Тело знало что делать без моей головы.

Может настоящая медитация — это не сидеть, а двигаться?

(танец и осознанность)

---
Based on: "медитация-перед-танцем", "ecstatic-dance-убуд"

[ Regenerate ] [ Save as draft ]
[ Insight ] [ Hot Take ] [ Question ] [ Story ] [ Framework ]
[ Save as example ]
```
...
| `/search keyword` | Текстовый поиск по заметкам |
| `/ask вопрос` | AI отвечает на основе твоих заметок (семантический поиск) |
| `/generate threads [тема]` | Генерация поста для Threads |
| `/project описание` | Создание проекта |
| `/music [описание]` | Музыкальная заметка или запись аудио |
| `/reindex` | Переиндексация всех заметок для AI-поиска |

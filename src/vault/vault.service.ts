...
// Upper bound on candidates pulled from Qdrant per note. Not a "max links" cap
// — a safety ceiling so an unbounded vault can't return thousands at once.
const RELATED_NOTES_FETCH_LIMIT = 50;
// Adaptive threshold. Instead of a fixed cosine cutoff (which drifts with text
// length/language), link every note within RELATED_NOTES_REL_RATIO of the best
// match for this note. RELATED_NOTES_MIN_SCORE is an absolute floor: if even
// the top match is below it, the note has no real relations and we link none.
const RELATED_NOTES_REL_RATIO = 0.85;
const RELATED_NOTES_MIN_SCORE = 0.35;
...
export class VaultService {
  ...
  async createFromClassification(
    ...
    // Append voice audio embed if saved with audio
    if (audioFileName) {
      finalContent += `\n\n![[${audioFileName}]]`;
    }

    // Append related notes as wikilinks (semantic retrieval via Qdrant)
    const relatedLinks = await this.findRelatedNoteLinks(content);
    if (relatedLinks.length) {
      finalContent += `\n\n> Связано: ${relatedLinks.map(n => `[[${n}]]`).join(' | ')}`;
    }
  ...
  // Find semantically related notes via Qdrant to embed as wikilinks.
  // No fixed count — links every note relevant relative to the best match, so
  // the Obsidian graph reflects all real connections without a magic cutoff.
  // Contacts are excluded (linked separately). Returns [] when the index is
  // empty, nothing is relevant, or embedding fails.
  private async findRelatedNoteLinks(content: string): Promise<string[]> {
    if (!this.embedding || content.trim().length < 10) return [];
    try {
      const hits = await this.embedding.searchSimilar(content, RELATED_NOTES_FETCH_LIMIT);
      const candidates = hits.filter(h => !h.docId.startsWith('contacts/'));
      // hits are sorted by score desc, so the first survivor is the best match.
      const topScore = candidates[0]?.score ?? 0;
      if (topScore < RELATED_NOTES_MIN_SCORE) return [];
      const cutoff = topScore * RELATED_NOTES_REL_RATIO;
      return candidates
        .filter(h => h.score >= cutoff)
        .map(h => h.docId.replace(/^[^/]+\//, '').replace(/\.md$/, ''));
    } catch {
      return [];
    }
  }
